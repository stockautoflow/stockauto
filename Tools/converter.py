...
def create_csv_from_excel(excel_path, output_dir):
    ...
    try:
        ...
        if pd.isna(ashi_type):
            ...
            return
        ashi_type = str(ashi_type).strip().upper() # 足種を大文字に統一して比較しやすくする
    ...
    for i in range(1, 51):
        ...
        try:
            ...
            if num_na_times > 0: # 時刻がNaTになっているものがある場合
                if ashi_type in ashi_types_to_fill_time: # ['D', 'W', 'M'] の場合
