...
from data_utils import load_csv_data as load_csv_data_from_utils # data_utils.py からインポートするものは別名にする
...
# === ヘルパー関数 ===
def load_csv_data(filepath): # base.py 内で直接使用する load_csv_data
    try:
        ...
        logging.error(f"    ファイルが見つかりません: {filepath}") # loggerを使用
        raise
    except Exception as e:
        logging.error(f"    CSVファイルの読み込みまたは前処理中にエラーが発生しました ({os.path.basename(filepath)}): {e}") # loggerを使用
        raise

def get_codes_from_data_dir(data_dir_path, base_interval_str_for_glob, date_str_for_glob):
...
def run_backtest(strategy_module_name, config_filepath,
    ...
    base_interval_str_for_file_search = base_interval_str_arg 
    ...
    for stock_idx, stock_code_val in enumerate(stock_codes_list_all):
        ...
        specific_base_file_path = os.path.join(DATA_DIR_PATH, f"{stock_code_val}_{base_interval_str_for_file_search}_{date_str}.csv")
        if os.path.exists(specific_base_file_path):
            filepath_base_data_val = specific_base_file_path
        else:
            alt_base_files_list = glob.glob(os.path.join(DATA_DIR_PATH, f"{stock_code_val}_{base_interval_str_for_file_search}_*.csv"))
        ...
        if filepath_base_data_val:
            ...
            logging.error(f"  {progress_str}: 基本足CSVファイル未検出 ({base_interval_str_for_file_search}基準)。スキップ。")
        ...
        try:
            df_base_data = load_csv_data(filepath_base_data_val) 
            ...
            if df_context_ohlc.empty and effective_context_interval > 0 :
    ...
    meta_info = {
        ...
        "base_data_interval": base_interval_str_for_file_search, 
        "data_date": date_str,
    ...
    meta_output_filename = f"BacktestMeta_{safe_strategy_name_for_file}_{interval_combination_str}_base{base_interval_str_for_file_search}_{date_str}_{now_str_for_files}.json"
