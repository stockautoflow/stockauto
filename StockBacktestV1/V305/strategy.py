...
CONFIG_FILENAME = "config.yaml" # 設定ファイル名

# DEFAULT_STRATEGY_PARAMS は空にする。すべてのデフォルト値は config.yaml で定義されるべき。
DEFAULT_STRATEGY_PARAMS = {}

def flatten_dict(d, parent_key='', sep='_'):
    """ネストした辞書をフラットな辞書に変換する。キーは親キーと結合され、大文字になる。"""
    items = []
    if isinstance(d, dict): # dが辞書の場合のみ処理
        for k, v in d.items():
            new_key = f"{parent_key}{sep}{k}" if parent_key else k
            if isinstance(v, dict):
                items.extend(flatten_dict(v, new_key, sep=sep).items())
            else:
                items.append((new_key.upper(), v)) # キーを大文字に統一
    return dict(items)

def load_strategy_config_yaml(filename):
    """
    ...
    if not os.path.exists(filename):
        ...
        try:
            with open(filename, 'r', encoding='utf-8') as f:
                ...
                if loaded_yaml and isinstance(loaded_yaml, dict): # ファイルが空でなく、辞書であることを確認
                    config_params_nested = loaded_yaml
                    print(f"[Strategy Info] Loaded parameters from '{filename}'.")
                elif not loaded_yaml:
                    print(f"[Strategy Warning] Config file '{filename}' is empty. All strategy parameters must be defined in this file.")
                else:
                    print(f"[Strategy Warning] Config file '{filename}' does not contain a valid YAML dictionary. All strategy parameters must be defined in this file.")
...
# グローバルスコープで設定ファイルを読み込む (フラットな辞書として)
# config.yaml が存在しない、または不正な場合は空の辞書になる
STRATEGY_PRIMARY_PARAMS = load_strategy_config_yaml(CONFIG_FILENAME)
...
def get_merged_param(key, params_from_backtester, strategy_primary_params_loaded,
    ...
    """
    パラメータを取得する。config.yaml の値を最優先し、なければバックテスターからの値を使用。
    どちらにもなければ警告を出し、型に応じた最小限のフォールバックまたはNoneを返す。
    キーは大文字で比較・取得する。
    """
    key_upper = key.upper()

    val_from_config = strategy_primary_params_loaded.get(key_upper)
    val_from_fw = params_from_backtester.get(key) # FWからのキーは元のケースを維持
    if val_from_fw is None: # FWにない場合は大文字でも試す
        val_from_fw = params_from_backtester.get(key_upper)

    val = val_from_config
    if val is None: # config.yaml になければ FW からの値を使用
        val = val_from_fw

    if val is None: # config.yaml にも FW からも取得できなかった場合
        print(f"[Strategy Critical Warning] Parameter '{key_upper}' not found in config.yaml or from backtester. "
              f"This may cause errors or unexpected behavior. Please define it in config.yaml.")
        # 型に応じた最小限のフォールバック (エラー回避のため、ただし理想的ではない)
        if is_activation_flag: return 0
        if is_bool: return False
        if is_period: return 20 # 例: 汎用的な期間
        if is_float: return 0.0
        return None # 文字列やその他の型は None
    ...
    try:
        ...
        return val # その他の型 (主に文字列)
    except (ValueError, TypeError) as e:
        print(f"[Strategy Warning] Parameter conversion error for key '{key_upper}', value '{val}' (type: {type(val)}): {e}. "
              "Returning minimal fallback. Please check config.yaml.")
        if is_activation_flag: return 0
        if is_bool: return False
...
# calculate_vwap, calculate_indicators, generate_signals, determine_exit_conditions 関数は
# 前回の回答から変更ありません。
# ただし、これらの関数内で get_merged_param を呼び出す際のキー名は、
# config.yaml の構造 (flatten_dict でフラット化・大文字化された後のキー名) に
# 合わせて正確に指定する必要があります。
...
def calculate_indicators(df_exec, df_context, params_from_backtester):
    def _get_param(key_name, is_period=False, is_float=False, is_activation_flag=False):
        # キー名は config.yaml のフラット化・大文字化されたキーと一致させる
        return get_merged_param(key_name, params_from_backtester, STRATEGY_PRIMARY_PARAMS,
    ...
    if not df_e.empty:
        ...
        for col_nan_e in ['STOCH_K_exec', 'STOCH_D_exec', 'MACD_exec', 'MACDsignal_exec', 'MACDhist_exec', 'BB_Middle_exec', 'BB_Upper_exec', 'BB_Lower_exec', f'ATR_{atr_exec_p_chart if atr_exec_p_chart else ""}_EXEC_Chart', 'VWAP_daily_exec']:
    ...
    if not df_c.empty: df_c_reindexed = df_c.reindex(df_e.index, method='ffill')
    ...
    else: df_merged = df_e.copy()

    if atr_ctx_p_for_sl:
        atr_col_for_sl_name_orig = f'ATR_{atr_ctx_p_for_sl}_ctx'
        if atr_col_for_sl_name_orig in df_c_reindexed.columns: df_merged[atr_col_for_sl_name_orig] = df_c_reindexed[atr_col_for_sl_name_orig]
        elif atr_col_for_sl_name_orig not in df_merged.columns: df_merged[atr_col_for_sl_name_orig] = np.nan

    if df_merged.empty and not df_e.empty: df_merged = df_e.copy()
    return df_merged

def generate_signals(df, params_from_backtester):
    ...
    def _get_param(key_name, is_period=False, is_float=False, is_activation_flag=False):
        return get_merged_param(key_name, params_from_backtester, STRATEGY_PRIMARY_PARAMS, is_period, is_float, is_activation_flag)
    ...
    required_cols_check_map = {
        "EMA_CTX_SHORT_ITS": (ema_short_col_its, use_ema_cross_ctx > 0), "EMA_CTX_LONG_ITS": (ema_long_col_its, use_ema_cross_ctx > 0),
        "ADX_CTX_ITS": (adx_col_its, use_adx_ctx > 0), "STOCH_K_EXEC": (stoch_k_col, use_stoch_entry > 0),
        "STOCH_D_EXEC": (stoch_d_col, use_stoch_entry > 0), "MACD_EXEC": (macd_line_col, use_macd_entry > 0),
        "MACD_SIGNAL_EXEC": (macd_signal_col, use_macd_entry > 0), "BB_MIDDLE_EXEC": (bb_middle_col, use_bb_middle_entry > 0),
    ...
    for display_name, (col_name, is_rule_active) in required_cols_check_map.items():
        if is_rule_active:
            if col_name is None or col_name not in df.columns: missing_or_all_nan_cols.append(f"{display_name} (column missing: expected '{col_name}')")
            elif df[col_name].isnull().all(): missing_or_all_nan_cols.append(f"{display_name} (column '{col_name}' is all NaN)")
    ...
    else: final_optional_long_cond = pd.Series(True, index=df.index)
    ...
    else: final_optional_short_cond = pd.Series(True, index=df.index)
    ...
    signals.loc[buy_signal_series] = 1.0; signals.loc[sell_signal_series] = -1.0
...
def determine_exit_conditions(current_position, current_bar_data, prev_bar_data,
                              params_from_backtester, entry_price=None, current_bar_time=None):
    if current_position == 0: return False, "", None

    # merged_params_for_exit は config.yaml (STRATEGY_PRIMARY_PARAMS) と FWパラメータで構成
    merged_params_for_exit = STRATEGY_PRIMARY_PARAMS.copy() # まずconfigから
    flat_fw_params = {k.upper(): v for k,v in params_from_backtester.items()} # FWもフラット大文字キー
    merged_params_for_exit.update(flat_fw_params) # FWの値で上書き（FW側で動的に変更したい場合など）


    def _get_exit_param(key_name_upper, is_float=False, is_activation_flag=False):
        val = merged_params_for_exit.get(key_name_upper)
        if val is None:
            print(f"[Strategy Critical Warning in Exit] Parameter '{key_name_upper}' not found. Ensure it's in config.yaml.")
            if is_activation_flag: return 0
            if is_float: return 0.0 # フォールバック値
            return None
        try:
            if is_activation_flag: return int(float(val))
            if is_float: return float(val)
            ...
            print(f"[Strategy Warning in Exit] Param convert error for key '{key_name_upper}', val '{val}'.")
            if is_activation_flag: return 0
            if is_float: return 0.0
            return None
    ...
    if sl_atr_multiple is not None and sl_atr_multiple > 0 and entry_price is not None and atr_col_for_sl and atr_col_for_sl in current_bar_data:
        ...
        if current_atr_val is not None and not np.isnan(current_atr_val) and current_atr_val > 0 and current_low is not None and not np.isnan(current_low) and current_high is not None and not np.isnan(current_high):
    ...
    if exit_reason_list: exit_now = True
    else:
        ...
        if tp_macd_hist_active > 0:
            ...
            if m_hist_curr is not None and m_hist_prev is not None and not (np.isnan(m_hist_curr) or np.isnan(m_hist_prev)):
                if current_position == 1 and m_hist_prev > 0 and m_hist_curr < m_hist_prev : exit_reason_list.append("TP:MACD_Hist_Reverse")
                elif current_position == -1 and m_hist_prev < 0 and m_hist_curr > m_hist_prev: exit_reason_list.append("TP:MACD_Hist_Reverse")
        if tp_bb_touch_active > 0:
            bb_upper = current_bar_data.get('BB_Upper_exec'); bb_lower = current_bar_data.get('BB_Lower_exec'); high_price = current_bar_data.get('High'); low_price = current_bar_data.get('Low')
            if bb_upper is not None and bb_lower is not None and high_price is not None and low_price is not None and not (np.isnan(bb_upper) or np.isnan(bb_lower) or np.isnan(high_price) or np.isnan(low_price)):
    ...
    if not exit_now and current_bar_time is not None:
        considers_force_exit = _get_exit_param('ACTIVATION_FLAGS_STRATEGY_CONSIDERS_FORCE_EXIT_TIME', is_activation_flag=True)
        force_exit_time_str_fw = _get_exit_param('TRADING_HOURS_FORCE_EXIT_TIME_STR')
        if considers_force_exit > 0 and force_exit_time_str_fw:
            try:
                force_exit_time_obj = datetime.datetime.strptime(force_exit_time_str_fw, '%H:%M:%S').time()
                if current_bar_time >= force_exit_time_obj:
                    if not exit_reason_list: exit_reason_list.append(f"強制決済({force_exit_time_obj.strftime('%H:%M')})"); exit_now = True
            except ValueError: pass
    exit_price_candidate = None
    final_exit_reason = ", ".join(sorted(list(set(exit_reason_list))))
